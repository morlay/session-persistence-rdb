  ...
  describe(`SessionPersistence contract: ${name}`, () => {
    ...
    it("crash recovery: load preserves an interrupted (unclosed) turn and closes it with turn/end {interrupted}", async () => {
      ...
      try {
        ...
        const afterInspect = (await persistence.listSnapshots()).find(
          (snapshot) => snapshot.header.id === m.id,
        )?.revision;
        ...
        const loaded = await persistence.load(m.id);
        const afterRepair = (await persistence.listSnapshots()).find(
          (snapshot) => snapshot.header.id === m.id,
        )?.revision;
    ...
    it("list() excludes a created-but-never-appended (zero-event) session", async () => {
      ...
      try {
        ...
        expect(
          (await persistence.listSnapshots()).map((snapshot) => snapshot.header.id),
    ...
    it("lists stable lightweight revisions that change after an append", async () => {
      ...
      try {
        ...
        const first = (await persistence.listSnapshots()).find(
          (snapshot) => snapshot.header.id === m.id,
        );
        const repeated = (await persistence.listSnapshots()).find(
          (snapshot) => snapshot.header.id === m.id,
        );
        ...
        const changed = (await persistence.listSnapshots()).find(
          (snapshot) => snapshot.header.id === m.id,
        );
    ...
    it("append rejects non-JSON-serializable event data, naming the event type", async () => {
      ...
      try {
        ...
        for (const [i, bad] of badValues.entries()) {
          ...
          await expect(persistence.append(mi.id, events)).rejects.toThrow(
